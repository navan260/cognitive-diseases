...
export default function PdfUpload() {
    ...
    const generatePDF = () => {
        ...
        doc.text("Dyslexia Platform - PDF Analysis Report", pageWidth / 2, 20, { align: "center" });
        ...
        addSection("Original Text", result.original);
        ...
        doc.save(`Dyslexia-PDF-Report-${new Date().toLocaleDateString()}.pdf`);
    };
        ...
        <div className={`dyslexia-wrapper ${isDyslexicFont ? "opendyslexic-font" : ""}`}>
            ...
            <main className="dys-sub-main">
                <header className="dys-sub-header">
                    <h1 className="dys-sub-title">📄 {t("uploadPdf")}</h1>
                    ...
                    <div className="dys-panel" style={{ maxWidth: '600px', width: '100%', margin: '0 auto', textAlign: 'center' }}>
                        <input 
                            ...
                            style={{ display: "none" }}
                        />
                        <label htmlFor="pdf-upload" className="font-helper-btn" style={{ fontSize: '1.1rem', padding: '20px', display: 'block', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: '12px' }}>
                            {file ? `🔔 ${t("selected")}: ${file.name}` : t("selectPdfFile")}
                        </label>
                        ...
                        {file && (
                            ...
                            >
                                {loading ? `⏳ ${t("extractingText")}...` : `🚀 ${t("processPdf")}`}
                            </button>
                        )}
                    </div>
                </header>
                ...
                <div className="dys-layout-grid">
                    <section className="dys-panel">
                        <h3 className="dys-panel-title">📝 {t("extractedContent")}</h3>
                        <div className="dys-text-box">
                            {result.original || <span style={{ opacity: 0.4 }}>{t("extractedContent")}...</span>}
                        </div>
                    </section>
                    ...
                    <section className="dys-panel">
                        ...
                        <div className="dys-text-box">
                            {loading ? (
                                <p style={{ opacity: 0.6 }}>⏳ {t("analyzingDocument")}</p>
                            ) : (
                                <>
                                    {activeTab === "summary" && (
